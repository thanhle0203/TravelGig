$(document).ready(function() {
  ...
  $.get("http://localhost:8282/searchHotelByName/" + searchText, {}, function(responseText) {
    $.each(responseText, function(key1, value1) {
      ...
      $.get("http://localhost:8383/api/reviews/" + value1.hotelId, {}, function(reviewResponse) {
        $.each(reviewResponse, function(key2, review) {
          reviewsHTML += "<li>" + review.review + "</li>";
        });

        var row = "<tr>" +
          "<td>" + hotelName + "</td>" +
          "<td><img class='hotelImage' src='" + imageURL + "' width='300' height='300'/></td>" +
          "<td>" + averagePrice + "</td>" +
          "<td>" + starRating + "</td>" +
          "<td><div class='reviewList'>" + 
            "<ul>" + reviewsHTML + "</ul>" +
          "</div></td>" +
          "<td>" + amenities + "</td>" +
          "</tr>";
    ...
    $("#tblHotel").on('click', '.hotelImage', function () {
		...
		$.get("http://localhost:8282/searchHotelByName/"+ searchText, {}, function(response) {
